...
export default function Page() {
  ...
  return <main className={`app-shell ${view === 'quiz' ? 'question-shell' : ''}`}>
    <header className="topbar">
      {view === 'home' ? <img className="brand brand-mark" src="/find-mark.svg" alt="find" /> : <button className="icon-button" aria-label={view === 'quiz' && index > 0 ? '上一题' : '返回首页'} onClick={() => view === 'quiz' && index > 0 ? setIndex((current) => current - 1) : setView('home')}><ArrowLeft size={21} /></button>}
      {view !== 'home' && <img className="brand brand-mark centered-brand" src="/find-mark.svg" alt="find" />}
    ...
    {view === 'home' && <>
      ...
      <div className="hero-art raised"><img src="/soft-hero.png" alt="粉色爱心与黄色、蓝色小花" fetchPriority="high" /></div>
    ...
    {view === 'result' && <>
      ...
      <section className="raised score-card"><div><p className="score-label">喜欢指数</p><div className="score-number">{result.total ?? '—'}<span>/ 100</span></div><p className="score-caption">来自你在 30 个日常情境中的选择。</p></div><div className="score-ring" style={{ '--score': `${result.total ?? 0}%` } as React.CSSProperties}><span><img src="/soft-hero.png" alt="" /></span></div></section>
